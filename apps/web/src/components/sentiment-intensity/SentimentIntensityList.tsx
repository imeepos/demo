...
export const SentimentIntensityList: React.FC<SentimentIntensityListProps> = ({
  ...
  const getIntensityColor = (intensity: number) => {
    if (intensity >= 0.7) return 'bg-green-100 text-green-800 border-green-300';
    if (intensity >= 0.4) return 'bg-yellow-100 text-yellow-800 border-yellow-300';
    return 'bg-red-100 text-red-800 border-red-300';
  };

  const getIntensityLabel = (intensity: number) => {
    if (intensity >= 0.7) return '高强度';
    if (intensity >= 0.4) return '中强度';
    return '低强度';
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                <Badge 
                  variant="outline" 
                  className={getIntensityColor(item.intensity)}
                >
                  {getIntensityLabel(item.intensity)}
                </Badge>
              </div>
              <div className="text-sm text-gray-600">
                强度值: {item.intensity.toFixed(2)}
              </div>
